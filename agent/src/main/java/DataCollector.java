    ...
    public void run() {
        try {
            ...
            while (!socket.isClosed()) {
                Agent.dataQueue.add(new DataEntity(new Timestamp(System.currentTimeMillis()), osBean.getCpuLoad(), osBean.getTotalMemorySize(), osBean.getFreeMemorySize(), path.getTotalSpace(), path.getUsableSpace()));
                // Add data
